...
public interface AmCoreIndexDao extends JpaRepository<AmCoreIndex, Long>, SearchAndPageRepository<AmCoreIndex, Long> {
	  ...
	  //汇总指标查询所有  ri
	  @Modifying
	  @Query(value = " select a.* from am_core_index a where a.group_id not like '3___' and a.group_id not like '4___' and a.group_id not like '2___' and a.index_id not like '2___'", nativeQuery = true)
	  ...
	  List<AmCoreIndex>findAllHeatBase();
		...
		@Query(value = " select a.* from aiam.am_core_index a where a.group_id ='3004' and a.index_name = ?1 ", nativeQuery= true)
