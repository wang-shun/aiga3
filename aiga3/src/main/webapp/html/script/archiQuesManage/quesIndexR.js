...
/*			Utils.getServerPage(srvMap.get(toptask), _topcmd, function(json, status) {//getArchDbConnectList
		...
		getDatabaseConnectTopList : function(){
			...
			_queryBtn.off('click').on('click', function() {	
				...
				var dbName = _form.find('select[name="db"]').val()
				if(dbName=='ZJCRMA'){
					_topcmd.indexId = COMMON_INDEX_ID_A;
					_topcmd.indexName = COMMON_INDEX_NAME;
				}else if(dbName=='ZJCRMB'){
					_topcmd.indexId = COMMON_INDEX_ID_B;
					_topcmd.indexName = COMMON_INDEX_NAME;
				}else if(dbName=='ZJCRMC'){
					_topcmd.indexId = COMMON_INDEX_ID_C;
					_topcmd.indexName = COMMON_INDEX_NAME;
				}else if(dbName=='ZJCRMD'){
					_topcmd.indexId = COMMON_INDEX_ID_D;
					_topcmd.indexName = COMMON_INDEX_NAME;
				}
				XMS.msgbox.show('数据加载中，请稍候...', 'loading');
