...
public class InspectMailSv extends BaseService {
	...
	public String creatInspectMailHtml(String time) throws ParseException {
		...
		html.append("<html>"+
			"<head><style>#mail p{margin: 0 0 5px;} #mail span{color: red;font-weight: bold;} #mail td{border: 1px solid #f4f4f4;font-size: 9pt;text-indent: 0;} #mail th{border: 1px solid #f4f4f4;border-bottom-width: 2px;text-indent: 0;text;text-align: left;} #mail .tableBody{width:100%;} #mail .tableTitle{font-weight: bold;text-align: center;font-size: 17.0pt;color:#333333;}</style></head><body>" +
		...
		for(InspectMailData inspectMailData:inspectMailDatas) {
			...
			if("标题".equals(inspectMailData.getModuleType())) {
				...
				html.append("<div style='margin-left: 39.0pt;margin-top:10px;font-weight: bold;'>");
				if(inspectMailData.getReportContent1() !=null) {
					html.append(inspectMailData.getReportContent1());
				}
				if(inspectMailData.getReportContent2() !=null) {
