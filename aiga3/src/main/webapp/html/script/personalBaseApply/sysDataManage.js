...
//					}
        ...
        _apply_save_event: function(_modal, planId) {
			...
			Rose.ajax.postJson(srvMap.get('thirdDataManageSave'),_cmd,function(json, status){
				if(status) {
					_modal.modal('hide');
					if(planId) {
						XMS.msgbox.show('文件已上传，添加成功！', 'success', 2000);
					} else {
		...
		_band_table_btn: function(sysId, type) {
			...
			//删除操作
			if(type == 'delete') {	
				XMS.msgbox.show('暂时不支持删除', 'success', 2000);
				return
