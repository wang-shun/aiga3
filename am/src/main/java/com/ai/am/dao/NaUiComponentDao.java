...
import com.ai.am.view.json.CommonCompTreeResponse;

public interface NaUiComponentDao extends
		SearchAndPageRepository<NaUiComponent, Long>,
		JpaRepository<NaUiComponent, Long> {
	...
	@Query(value = "select sys_id as id, 0 as pid, sys_name as name from aiga_system_folder where is_invalid=0 and is_invalid is not null "
			+" and sys_id in(select sys_id from aiga_sub_sys_folder where subsys_id in(select sub_sys_id from aiga_fun_folder where fun_id in(select parent_id from na_ui_component)))"
			+ " union all select subsys_id as id, sys_id as pid, sys_name as name from aiga_sub_sys_folder "
			+"where subsys_id in(select sub_sys_id from aiga_fun_folder where fun_id in(select parent_id from na_ui_component))"
			...
			+ "and fun_id in(select parent_id from na_ui_component)"
