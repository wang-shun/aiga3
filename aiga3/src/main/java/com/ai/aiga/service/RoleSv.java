...
import com.ai.aiga.service.staff.StaffConstant;
...
public Object list(int pageNumber, int pageSize ,SysRole condition ) throws ParseException {
	...
	public void saveFouraRole(FouraRoleRequest roleRequest) {
		if(roleRequest == null){ 
			BusinessException.throwBusinessException(ErrorCode.Parameter_null);
		}
		if(StringUtils.isBlank(roleRequest.getName())){
		...
		SysRole sysRole = BeanMapper.map(roleRequest, SysRole.class);
		sysRole.setCreateDate(new Date(System.currentTimeMillis()));
		sysRoleDao.save(sysRole);
	}
	
	public void updateFouraRole(SysRole roleRequest) {
		if(roleRequest == null){ 
			BusinessException.throwBusinessException(ErrorCode.Parameter_null);
		}
		if(StringUtils.isBlank(roleRequest.getCode())){
		...
		roleRequest.setCreateDate(new Date(System.currentTimeMillis()));
		sysRoleDao.save(roleRequest);
	}
