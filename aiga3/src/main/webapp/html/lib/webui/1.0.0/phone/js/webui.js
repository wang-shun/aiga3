...
;(function(){
	...
	* @constructor picker
	...
	$.extend({
		...
		genMonthfield:function(_year, _month){
			
			var month={'id':'month','content':[],'reload':false};
			month.content = function(datas){
				var _con = [];
				if(datas['year'] == _year){
					for(var i = _month;i <= 12;i++){
						_con.push({'value':i,'text':i});
					}
				} else {
					for(var i = 1;i <= 12;i++){
						_con.push({'value':i,'text':i});
		...
		genDayfield:function(_year, _month, _day){
			...
			day.content=function(datas){
				...
				var content=[];
				if(datas['year'] == _year && datas['month'] == _month){
					for(var i = _day;i<=maxDay;i++){
	...
	$.fn.extend({
		...
		AIDtPicker:function(param){
			...
			var fields=[];
			param = $.extend({},{
				beginData: "2010-01-01"
			},param);
			var _d = param.beginData.split("-");
			var _year = _d[0] || param.beginYear,
				_month = _d[1].replace(/\b(0+)/gi,"") || 1,
				_day = _d[2].replace(/\b(0+)/gi,"") || 1;
			if(param.type.toLowerCase()=='date'){
				fields.push($.genYearfield(param.beginYear,param.endYear));
				fields.push($.genMonthfield(_year, _month));
				fields.push($.genDayfield(_year, _month, _day));
			}
			if(param.type.toLowerCase()=='time'){
				fields.push($.genHourfield());
				fields.push($.genMinutefield(param.beginMonth, param.endMonth));
			}
