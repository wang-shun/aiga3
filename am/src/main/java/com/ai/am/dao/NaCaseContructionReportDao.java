...
import com.ai.am.domain.NaCaseConstructionReport;
...
public interface NaCaseContructionReportDao extends JpaRepository<NaCaseConstructionReport, Long>, SearchAndPageRepository<NaCaseConstructionReport, Long>{
	...
	@Query(value = "SELECT SYS_ID, COUNT(1) FROM (SELECT S.SYS_ID, F.FUN_ID FROM NA_TEST_CASE A, AIGA_FUN_FOLDER F, AIGA_SYSTEM_FOLDER S WHERE A.FUN_ID = F.SYS_ID AND F.SYS_ID = S.SYS_ID AND S.IMPORTANT_CLASS = 1 AND F.IMPORTANT_CLASS IN (1, 2, 3) AND A.IMPORTANT IN (1, 2, 3)  GROUP BY S.SYS_ID, F.FUN_ID) GROUP BY SYS_ID", nativeQuery = true)
	List<Object> findSysFunCover();
	@Query(value = " SELECT SYS_ID, COUNT(1) FROM (SELECT S.SYS_ID, A.TEST_ID FROM NA_TEST_CASE A, AIGA_FUN_FOLDER F, AIGA_SYSTEM_FOLDER S WHERE A.FUN_ID = F.SYS_ID AND F.SYS_ID = S.SYS_ID AND S.IMPORTANT_CLASS = 1 AND F.IMPORTANT_CLASS IN (1, 2, 3) AND A.IMPORTANT IN (1, 2, 3)  AND A.CREATE_TIME < (ADD_MONTHS(TRUNC(TO_DATE(?1,'YYYYMM'),'MM'),1)) GROUP BY S.SYS_ID, A.TEST_ID) GROUP BY SYS_ID", nativeQuery = true)
	List<Object> findSysCaseCountA(String dayOrMonth);
	
	@Query(value = " SELECT SYS_ID, COUNT(1) FROM (SELECT S.SYS_ID, A.TEST_ID FROM NA_TEST_CASE A, AIGA_FUN_FOLDER F, AIGA_SYSTEM_FOLDER S WHERE A.FUN_ID = F.SYS_ID AND F.SYS_ID = S.SYS_ID AND S.IMPORTANT_CLASS = 1 AND F.IMPORTANT_CLASS IN (1, 2, 3) AND A.IMPORTANT IN (1, 2, 3)  AND A.CREATE_TIME < (TRUNC(TO_DATE(?1,'YYYYMMDD'),'DD')) GROUP BY S.SYS_ID, A.TEST_ID) GROUP BY SYS_ID", nativeQuery = true)
	List<Object> findSysCaseCountB(String dayOrMonth);

	@Query(value = " SELECT SYS_ID, COUNT(1) FROM (SELECT S.SYS_ID, A.AUTO_ID FROM NA_AUTO_CASE A, NA_TEST_CASE B, AIGA_FUN_FOLDER F, AIGA_SYSTEM_FOLDER S WHERE A.TEST_ID = B.TEST_ID AND A.FUN_ID = F.SYS_ID AND F.SYS_ID = S.SYS_ID AND S.IMPORTANT_CLASS = 1 AND F.IMPORTANT_CLASS IN (1, 2, 3) AND A.IMPORTANT IN (1, 2, 3) AND A.ENVIRONMENT_TYPE = ?1 AND A.UPDATE_TIME < (ADD_MONTHS(TRUNC(TO_DATE(?2,'YYYYMM'),'MM'),1)) AND A.STATUS = 1 GROUP BY S.SYS_ID, A.AUTO_ID) GROUP BY SYS_ID", nativeQuery = true)
	...
	void delete(Long type, String currentMonth);
	...
	@Query(value = "SELECT BUSI_ID ,COUNT(1) FROM (SELECT F.BUSI_ID, T.TEST_ID FROM NA_TEST_CASE T, (SELECT  B.BUSI_ID,A.FUN_ID　FROM (SELECT DISTINCT TRIM(REGEXP_SUBSTR(DECODE(SUBSTR(BUSI_LABEL, -1), ',', SUBSTR(BUSI_LABEL, 1, LENGTH(BUSI_LABEL) - 1),　BUSI_LABEL), '[^,]+', 1, LEVEL)) LAB, AI.FUN_ID, AI.IS_INVALID, AI.SYS_NAME,　AI.IMPORTANT_CLASS FROM AIGA_FUN_FOLDER AI CONNECT BY LEVEL <= LENGTH(DECODE(SUBSTR(BUSI_LABEL, -1),　',',  SUBSTR(BUSI_LABEL,  1, LENGTH(BUSI_LABEL) - 1), BUSI_LABEL)) - LENGTH(REPLACE(BUSI_LABEL, ',')) + 1　AND PRIOR FUN_ID = FUN_ID AND PRIOR DBMS_RANDOM.VALUE IS NOT NULL) A, NA_BUSINESS B WHERE A.LAB = B.BUSI_NAME AND A.IS_INVALID = 0 AND B.INVALID = 0 AND B.IMPORTANT IN (1, 2, 3) ) F WHERE  T.FUN_ID = F.FUN_ID AND  T.IMPORTANT IN (1, 2, 3) AND (T.CREATE_TIME IS NULL OR T.CREATE_TIME < (ADD_MONTHS(TRUNC(TO_DATE(?1,'YYYYMM'),'MM'),1)) )   GROUP BY F.BUSI_ID,T.TEST_ID ) GROUP BY BUSI_ID", nativeQuery = true)
	List<Object> findBusiCaseCountA(String dayOrMonth);
	
	@Query(value = "SELECT BUSI_ID ,COUNT(1) FROM (SELECT F.BUSI_ID, T.TEST_ID FROM NA_TEST_CASE T, (SELECT  B.BUSI_ID,A.FUN_ID　FROM (SELECT DISTINCT TRIM(REGEXP_SUBSTR(DECODE(SUBSTR(BUSI_LABEL, -1), ',', SUBSTR(BUSI_LABEL, 1, LENGTH(BUSI_LABEL) - 1),　BUSI_LABEL), '[^,]+', 1, LEVEL)) LAB, AI.FUN_ID, AI.IS_INVALID, AI.SYS_NAME,　AI.IMPORTANT_CLASS FROM AIGA_FUN_FOLDER AI CONNECT BY LEVEL <= LENGTH(DECODE(SUBSTR(BUSI_LABEL, -1),　',',  SUBSTR(BUSI_LABEL,  1, LENGTH(BUSI_LABEL) - 1), BUSI_LABEL)) - LENGTH(REPLACE(BUSI_LABEL, ',')) + 1　AND PRIOR FUN_ID = FUN_ID AND PRIOR DBMS_RANDOM.VALUE IS NOT NULL) A, NA_BUSINESS B WHERE A.LAB = B.BUSI_NAME AND A.IS_INVALID = 0 AND B.INVALID = 0 AND B.IMPORTANT IN (1, 2, 3) ) F WHERE  T.FUN_ID = F.FUN_ID AND  T.IMPORTANT IN (1, 2, 3) AND (T.CREATE_TIME IS NULL OR T.CREATE_TIME < (TRUNC(TO_DATE(?1,'YYYYMMDD'),'DD')) )   GROUP BY F.BUSI_ID,T.TEST_ID ) GROUP BY BUSI_ID", nativeQuery = true)
	List<Object> findBusiCaseCountB(String dayOrMonth);
	
	@Query(value = "SELECT BUSI_ID ,COUNT(1) FROM (SELECT F.BUSI_ID, T.AUTO_ID FROM NA_AUTO_CASE T, NA_TEST_CASE S, (SELECT  B.BUSI_ID,A.FUN_ID　FROM (SELECT DISTINCT TRIM(REGEXP_SUBSTR(DECODE(SUBSTR(BUSI_LABEL, -1), ',', SUBSTR(BUSI_LABEL, 1, LENGTH(BUSI_LABEL) - 1),　BUSI_LABEL), '[^,]+', 1, LEVEL)) LAB, AI.FUN_ID, AI.IS_INVALID, AI.SYS_NAME,　AI.IMPORTANT_CLASS FROM AIGA_FUN_FOLDER AI CONNECT BY LEVEL <= LENGTH(DECODE(SUBSTR(BUSI_LABEL, -1),　',',  SUBSTR(BUSI_LABEL,  1, LENGTH(BUSI_LABEL) - 1), BUSI_LABEL)) - LENGTH(REPLACE(BUSI_LABEL, ',')) + 1　AND PRIOR FUN_ID = FUN_ID AND PRIOR DBMS_RANDOM.VALUE IS NOT NULL) A, NA_BUSINESS B WHERE A.LAB = B.BUSI_NAME AND A.IS_INVALID = 0 AND B.INVALID = 0 AND B.IMPORTANT IN (1, 2, 3) ) F WHERE  T.TEST_ID = S.TEST_ID AND T.FUN_ID = F.FUN_ID AND  T.IMPORTANT IN (1, 2, 3) AND (T.UPDATE_TIME IS NULL OR T.UPDATE_TIME < (ADD_MONTHS(TRUNC(TO_DATE(?1,'YYYYMM'),'MM'),1)) ) AND T.ENVIRONMENT_TYPE = ?2 AND T.STATUS = 1 GROUP BY F.BUSI_ID,T.AUTO_ID ) GROUP BY BUSI_ID", nativeQuery = true)
	List<Object> findBusiAutoA(String dayOrMonth, Long type);
	
	@Query(value = "SELECT BUSI_ID ,COUNT(1) FROM (SELECT F.BUSI_ID, T.AUTO_ID FROM NA_AUTO_CASE T, NA_TEST_CASE S, (SELECT  B.BUSI_ID,A.FUN_ID　FROM (SELECT DISTINCT TRIM(REGEXP_SUBSTR(DECODE(SUBSTR(BUSI_LABEL, -1), ',', SUBSTR(BUSI_LABEL, 1, LENGTH(BUSI_LABEL) - 1),　BUSI_LABEL), '[^,]+', 1, LEVEL)) LAB, AI.FUN_ID, AI.IS_INVALID, AI.SYS_NAME,　AI.IMPORTANT_CLASS FROM AIGA_FUN_FOLDER AI CONNECT BY LEVEL <= LENGTH(DECODE(SUBSTR(BUSI_LABEL, -1),　',',  SUBSTR(BUSI_LABEL,  1, LENGTH(BUSI_LABEL) - 1), BUSI_LABEL)) - LENGTH(REPLACE(BUSI_LABEL, ',')) + 1　AND PRIOR FUN_ID = FUN_ID AND PRIOR DBMS_RANDOM.VALUE IS NOT NULL) A, NA_BUSINESS B WHERE A.LAB = B.BUSI_NAME AND A.IS_INVALID = 0 AND B.INVALID = 0 AND B.IMPORTANT IN (1, 2, 3) ) F WHERE  T.TEST_ID = S.TEST_ID AND T.FUN_ID = F.FUN_ID AND  T.IMPORTANT IN (1, 2, 3) AND (T.UPDATE_TIME IS NULL OR T.UPDATE_TIME < (TRUNC(TO_DATE(?1,'YYYYMMDD'),'DD')) ) AND T.ENVIRONMENT_TYPE = ?2 AND T.STATUS = 1 GROUP BY F.BUSI_ID,T.AUTO_ID ) GROUP BY BUSI_ID", nativeQuery = true)
	List<Object> findBusiAutoB(String dayOrMonth, Long type);
}
