...
public class ArchFunctionRecordSv extends BaseService {
	...
	public List<TopDataOutPut> queryTopData(String type) {
		...
		} else if ("userNameWeek".equals(type)){
		...
		} else if ("menuNameMonth".equals(type)) {
			sql = "select * from (SELECT t.menu_name as data_name,t.menu_code,t.menu_name, t.menu_url ,count(*) as data_num FROM ARCH_FUNCTION_RECORD t where sysdate-30 <= t.record_time group by t.menu_name,t.menu_code,t.menu_name, t.menu_url order by count(*) DESC) where rownum <= 10";
		} else {
