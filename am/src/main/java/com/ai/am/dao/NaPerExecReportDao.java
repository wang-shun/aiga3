...
import com.ai.am.domain.NaPerExecReport;
...
public interface NaPerExecReportDao extends JpaRepository<NaPerExecReport, Long>, SearchAndPageRepository<NaPerExecReport, Long>{
	...
	@Query(value = "select ceil((max(t1.done_date) - min(t1.done_date)) * 86400 * 1000), t5.name, t5.code from na_plan_case_result t1 join na_online_task_distribute t2 on t1.sub_task_id = t2.task_id join na_online_task_distribute t3 on t2.parent_task_id = t3.task_id join na_change_plan_onile t4 on t3.online_plan = t4.online_plan left join aiga_staff t5 on t2.deal_op_id = t5.staff_id where t4.online_plan = ?1 and t2.task_type = 1 and t1.done_date is not null group by t5.name, t5.code", nativeQuery = true)
