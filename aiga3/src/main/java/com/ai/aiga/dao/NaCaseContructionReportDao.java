...
public interface NaCaseContructionReportDao extends JpaRepository<NaCaseConstructionReport, Long>, SearchAndPageRepository<NaCaseConstructionReport, Long>{
	...
	@Query(value = " SELECT SYS_ID, COUNT(1) FROM (SELECT S.SYS_ID, A.AUTO_ID FROM NA_AUTO_CASE A, AIGA_FUN_FOLDER F, AIGA_SYSTEM_FOLDER S WHERE A.FUN_ID = F.SYS_IDAND F.SYS_ID = S.SYS_ID AND S.IMPORTANT_CLASS = 1 AND F.IMPORTANT_CLASS IN (1, 2, 3) AND A.IMPORTANT IN (1, 2, 3) AND A.ENVIRONMENT_TYPE = ?1 AND A.UPDATE_TIME < (?2) GROUP BY S.SYS_ID, A.AUTO_ID) GROUP BY SYS_ID", nativeQuery = true)
	List<Object> findSysCaseCount(Long type, String dayOrMonth);

	@Query(value = " SELECT SYS_ID, COUNT(1) FROM (SELECT S.SYS_ID, A.AUTO_ID FROM NA_AUTO_CASE A, AIGA_FUN_FOLDER F, AIGA_SYSTEM_FOLDER S WHERE A.FUN_ID = F.SYS_IDAND F.SYS_ID = S.SYS_ID AND S.IMPORTANT_CLASS = 1 AND F.IMPORTANT_CLASS IN (1, 2, 3) AND A.IMPORTANT IN (1, 2, 3) AND A.ENVIRONMENT_TYPE = ?1 AND A.UPDATE_TIME < (?2) AND A.HAS_AUTO = 1 GROUP BY S.SYS_ID, A.AUTO_ID) GROUP BY SYS_ID", nativeQuery = true)
	List<Object> findSysAuto(Long type, String dayOrMonth);
